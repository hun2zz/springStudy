...
fields.forEach(field => {
    const $passCk1 = document.getElementById('password')
    const $passCk = document.getElementById('password_check')
    const $input = document.getElementById(field.id); // 입력 요소 가져오기
    $input.addEventListener('keyup', debounce(async (e) => { // 키보드 입력 시마다 유효성 검증
        if(field.id === 'password' && $passCk.value.trim() ) {
            const isValid = await [...fields][2].validator($passCk.value); // 유효성 검증 함수 호출
            const $errorSpan = document.getElementById([...fields][2].errorElement);
